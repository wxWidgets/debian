...
// redrawn and sets minimum size hints on the inner scrolled area.  Scrollbars
// are sutomatically set to accomodate this and the ScrollRate.
//
// Note this class should produce identical results to the one above.
...
public:
    MyScrolledWindowSmart(wxWindow *parent) : MyScrolledWindowBase(parent)
    {
        ...
        SetVirtualSizeHints( -1, ( m_nLines + 1 ) * m_hLine );
    }
    ...
    : wxScrolledWindow( parent )
{
    ...
    // Populate and 'set' sizer with a resizing button and other
    // static decoration
    ...
    innersizer->SetVirtualSizeHints( this );
}

void MyAutoScrollWindow::OnResizeClick( wxCommandEvent &WXUNUSED( event ) )
{
    wxLogDebug( _T("Button Size: (%d, %d)"),
                m_button->GetSize().GetWidth(),
                m_button->GetSize().GetHeight() );
    ...
    if( m_button->GetSize() == SMALL_BUTTON )
    {
        ...
        // FIXME: Bug #1  wxADJUST_MINSIZE is ignored/ineffective, without this
        //                the button will never get smaller.
	//        Remedy: 'Best' size and 'minimum' size are neither synonymous
	//                nor well synchronised. --  CalcMin uses BestSize,
	//                either change that, or fix GetBestSize (everywhere),

        //GetSizer()->SetItemMinSize( m_button, LARGE_BUTTON.GetWidth(), LARGE_BUTTON.GetHeight() );
    }
    else
    {
        m_button->SetSizeHints( SMALL_BUTTON.GetWidth(), SMALL_BUTTON.GetHeight() );
        //GetSizer()->SetItemMinSize( m_button, SMALL_BUTTON.GetWidth(), SMALL_BUTTON.GetHeight() );
    }

    // FIXME: Bug #2  The VirtualSize will never get smaller in a call to Layout
    //                unless we do this first.  Under MSW it's still insufficent,
    //                the virtual area will not be udpated until a size event occurs.
    SetVirtualSizeHints( -1, -1 );

    // Then re-layout the sizer and adjust the scrolled area

    Layout();
    GetSizer()->SetVirtualSizeHints( this );

    // FIXME: Bug #3 wxNSW does not correctly update the srollbars
    //               without this here.  Doesn't do much better with
    //               it either though.  Scroll the window while the
    //               button is large, there will be an offset error in
    //               the redrawn canvas when it is made smaller again.
    //               wxGtk does not suffer this particular problem.
    AdjustScrollbars();
}
                  ...
                  wxPoint(20,20), wxSize(470,500) )
{
    ...
    // No need to call SetAutoLayout anymore, SetSizer will do it.
